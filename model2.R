...
glmdata$NoPGroup <- cut(glmdata$Number.of.Persons,
                        breaks = c(-Inf, 2, 5, 15, 50, Inf),
                        labels = c("01_<2", "02_2_5", "03_5-15", "04_15-50", "05_>=50"),
                        right = FALSE)
...
glmdata$EmployedGroup <- cut(glmdata$Num_of_ft_Employee,
                             breaks = c(-Inf,1,3,25,Inf),
                             labels = c("01_<1b","02<1-3","03_3-25","04_>=25"), 
                             right=FALSE)
...
#Outlier analysis##############################
plot(glmdata$Num_of_ft_Employee) #one clear outlier

#Age##########################################
myFun1("AgeGroup","ClaimCost")
myFun2("AgeGroup","ClaimCost","CompanyAge")
plot(glmdata$CompanyAge)

#Employed######################################
myFun1("EmployedGroup","ClaimCost")
myFun2("EmployedGroup","ClaimCost","Num_of_ft_Employee") 
...
glmdata$Financial.Rating[glmdata$Financial.Rating == "AN"] <- "C" #too little data, so join
...
glmdata2 <- aggregate(glmdata[c("Duration", "NumberOfClaims", "ClaimCost")],by=list(NoP_group = glmdata$NoPGroup,
                                                                                    ...
                                                                                    Financial_group = as.factor(glmdata$Financial.Rating),
                                                                                    #Employed_group = as.factor(glmdata$EmployedGroup),
                                                                                    Age_group = as.factor(glmdata$AgeGroup)
...
new.cols <-
  foreach (rating.factor = c("NoP_group", 
                             "Activity_group",
                             "Financial_group",
                             #"Employed_group",
                             "Age_group"
...
#LR testing########################################
model.base <- glm(NumberOfClaims ~ NoP_group + Activity_group + offset(log(Duration)),
                  data = glmdata2, family = poisson)

lrtest(model.frequency, model.base) #adding Age group is beneficial
#AIC testing#######################################
AIC(model.base)
AIC(model.frequency) #less overfit :D
#Goodness of fit testing###########################
pchisq(model.frequency$deviance, df=model.frequency$df.residual, lower.tail=FALSE) #bad fit?
pchisq(model.base$deviance, df=model.base$df.residual, lower.tail=FALSE) #even worse
#Residual plotting#################################
plot(model.frequency, which=2)
...
variableLevels <- c(nlevels(glmdata2[["NoP_group"]]),
                    ...
                    #nlevels(glmdata2[["Employed_group"]]),
...
model.severity <-
  glm(avgclaim ~ NoP_group + 
        Activity_group + 
        offset(log(Duration)) + 
        Financial_group +
        #Employed_group +
        Age_group, 
      data = glmdata2[glmdata2$avgclaim>0,], family = Gamma("log"), weight=NumberOfClaims, maxit=100)

#LR testing########################################
model.base <- glm(avgclaim ~ NoP_group + Activity_group + offset(log(Duration)),
                  data = glmdata2[glmdata2$avgclaim>0,], family = Gamma("log"), weight=NumberOfClaims, maxit=100)

lrtest(model.severity, model.base) #adding Age group is beneficial
#AIC testing#######################################
AIC(model.base)
AIC(model.severity) #less overfit :D?
#Goodness of fit testing###########################
pchisq(model.severity$deviance, df=model.severity$df.residual, lower.tail=FALSE) #bad fit?
pchisq(model.base$deviance, df=model.base$df.residual, lower.tail=FALSE) #even worse
#Residual plotting#################################
plot(model.severity, which=2)
...
p7 <- ggplot(subset(glmdata3, rating.factor=="EmployedGroup"), aes(x=class, y=rels.frequency)) +
  geom_point(colour="blue") + geom_line(aes(group=1), colour="blue") + ggtitle("Employed: frequency factors") +
  geom_text(aes(label=paste(round(rels.frequency,2))), nudge_y=0.05)

p8 <- ggplot(subset(glmdata3, rating.factor=="EmployedGroup"), aes(x=class, y=rels.severity)) +
  geom_point(colour="blue") + geom_line(aes(group=1), colour="blue") + ggtitle("Employed: severity factors") +
  geom_text(aes(label=paste(round(rels.severity,2))), nudge_y=0.1)

p9 <- ggplot(subset(glmdata3, rating.factor=="EmployedGroup"), aes(x=class, y=rels.risk)) +
  geom_point(colour="blue") + geom_line(aes(group=1), colour="blue") + ggtitle("Employed: risk factors") +
  geom_text(aes(label=paste(round(rels.risk,2))), nudge_y=0.1)
...
#Hard to say, but let's say it'll be 50% more than last year
Expected_ClaimCost = Claims_Per_Year$ClaimCost[Claims_Per_Year$Category==2020] *1.5 
